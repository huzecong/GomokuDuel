...
CustomPage {
	...
	function loadGame(isHost, ip, id, p1Name, p1Avatar, p2Name, p2Avatar, myself) {
		console.log(isHost, ip, id, p1Name, p1Avatar, p2Name, p2Avatar, myself)
		GameStorage.isHost = isHost
		GameStorage.opponentIP = ip
		GameStorage.opponentUniqueId = id
	...
	BroadcastManager {
		...
		onNewOpponent: {
			...
			createMatchDialog.uniqueId = uniqueId
			createMatchDialog.avatarId = avatarId
		...
		onRequestAccepted: {
			if (!connectToHostDialog.done) {
				...
				loadGame(manager.isHost,
						 connectToHostDialog.opponentIP,
						 connectToHostDialog.uniqueId,
						 connectToHostDialog.profileName,
						 connectToHostDialog.avatarId,
		...
		onConnectionConfirmed: {
			...
			loadGame(manager.isHost,
					 createMatchDialog.opponentIP,
					 createMatchDialog.uniqueId,
					 GameStorage.profileName,
					 GameStorage.avatarId,
					 createMatchDialog.profileName,
					 createMatchDialog.avatarId, 0)
		}
	}
...
//				keyNavigationWraps: true
				...
				delegate: CustomSubtitled {
					...
					property int rounds: GameStorage.rounds(model.uniqueId)
					property double winningRate: GameStorage.winningRate(model.uniqueId)
...
with a winning rate of %2%.").arg(Number(rounds)).arg(Number(winningRate).toFixed(1))
					...
					onClicked: {
						if (list.currentIndex == index) {
							...
							connectToHostDialog.showDialog(model.name, model.ip, model.avatarId, model.uniqueId)
						} else {
